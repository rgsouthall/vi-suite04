...
#    objno = len([obj for obj in bpy.data.objects if obj.layers[1] == True])
    ...
    for line in resfile:
        if len(line.split(",")) > 2 and line.split(",")[2] == "Day of Simulation[]":
            dos[0] = line.split(",")[0]
            xtypes = [("0", "Time", "Plot time on the x-axis")]
            ...
            if line.split(",")[3].strip('\n') == "Site Direct Solar Radiation Rate per Area [W/m2] !Hourly":
                ...
                asb[0] = line.split(",")[0]
            xtypes.append(("1", "Climate", "Plot a climate parameter on the x-axis"))
            ...
            if ("2", "Zone", "Plot a zone result on the x-axis") not in xtypes:
                xtypes.append(("2", "Zone", "Plot a zone result on the x-axis"))     
