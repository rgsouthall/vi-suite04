...
#    deh = bpy.props.IntProperty(name = "End", description = "", min = 1, max = 24, default = 24)
    ...
    parametricmenu = bpy.props.EnumProperty(items=pmitems,
                name="", description="Parametric result display", update=aupdate)
...
#            if self.parametricmenu == '2' or innode.bl_label in ('VI Location', 'EnVi Simulation'): 
            row = layout.row()    
            row.label(label)
            row.prop(self, '["Start"]')
            row.prop(self, '["End"]')
...
class EnViZone(bpy.types.Node, EnViNodes):
    ...
    def zupdate(self, context):
        ...
        for oname in [outputs for outputs in self.outputs if outputs.bl_idname in ('EnViBoundSocket', 'EnViSFlowSocket', 'EnViSSFlowSocket')]:
            self.outputs.remove(oname)
        for iname in [inputs for inputs in self.inputs if inputs.bl_idname in ('EnViBoundSocket', 'EnViSFlowSocket', 'EnViSSFlowSocket')]:
